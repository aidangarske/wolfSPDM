    ...
    const byte* peerPubKeyX, const byte* peerPubKeyY)
{
    ...
    if (ctx->sharedSecretSz < WOLFSPDM_ECC_KEY_SIZE) {
        word32 padLen = WOLFSPDM_ECC_KEY_SIZE - ctx->sharedSecretSz;
        XMEMMOVE(ctx->sharedSecret + padLen, ctx->sharedSecret, ctx->sharedSecretSz);
        XMEMSET(ctx->sharedSecret, 0, padLen);
    ...
    byte* sig, word32* sigSz)
{
    ...
    if (rLen < WOLFSPDM_ECC_KEY_SIZE) {
        XMEMMOVE(sig + (WOLFSPDM_ECC_KEY_SIZE - rLen), sig, rLen);
        XMEMSET(sig, 0, WOLFSPDM_ECC_KEY_SIZE - rLen);
    }
    if (sLen < WOLFSPDM_ECC_KEY_SIZE) {
        XMEMMOVE(sig + WOLFSPDM_ECC_KEY_SIZE + (WOLFSPDM_ECC_KEY_SIZE - sLen),
                 sig + WOLFSPDM_ECC_KEY_SIZE, sLen);
